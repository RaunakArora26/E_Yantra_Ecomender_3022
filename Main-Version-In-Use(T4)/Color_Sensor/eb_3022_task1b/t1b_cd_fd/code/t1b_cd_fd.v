...
module t1b_cd_fd (
    ...
    output reg S_2,S_3,
	 output reg E12_R = 0,E12_G = 0,E12_B = 0,
...
initial begin // editing this initial block is allowed
    {S_2,S_3} = 2'b00; color = 0;counter = -1;state = 0;counter_cs_out = 0;clk_3125MHz = 0;
end
...
begin
	if (!counterr) clk_3125MHz <= ~clk_3125MHz; // toggles clock signal
    counterr <= counterr + 1'b1;
    if(counterr == 5'd7)    counterr <= 4'b0;
end


always@(posedge clk_3125MHz)
begin
	case(state)
		S0:begin
...
always @(posedge clk_3125MHz) begin
	case(color)
		//2'b00: {E12_R,E12_G,E12_B} <= 3'b000;
		2'b01: {E12_R,E12_G,E12_B} <= 3'b100;
		2'b10: {E12_R,E12_G,E12_B} <= 3'b010;
		2'b11: {E12_R,E12_G,E12_B} <= 3'b001;
	endcase
end
